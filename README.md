...
### 2. Configure Codex as a standalone MCP server

Add the following to `~/.codex/config.toml`, replacing the two absolute paths:
...
### 3. Use as a Codex plugin bundle
...
These files form a valid Codex plugin bundle. A marketplace maintainer can place this directory under `plugins/wolt` and reference it from `.agents/plugins/marketplace.json`. The project does not modify your personal Codex installation or marketplace automatically.
...
3. A fresh standalone esbuild bundle.
...
├── .mcp.json                   # Bundled MCP launch configuration
├── dist/server.js              # Self-contained runtime bundle
├── skills/wolt/                # Codex skill and API-family references
├── specs/wolt-official.json    # Captured official Wolt contracts
├── src/                        # TypeScript MCP implementation
├── tests/                      # Route, schema, safety, HTTP, and MCP tests
└── scripts/smoke.mjs           # Dependency-free stdio smoke test
```
...
- Validate the Codex plugin and `skills/wolt` metadata.
